...
import { runBenchmark, formatTime, formatBytes, getPerformanceGrade } from '../utils/performance';
...
const Dashboard: React.FC = () => {
    ...
    return (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: 24, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                        <thead>
                            <tr style={{ backgroundColor: '#f1f5f9' }}>
                                <th style={{ padding: 12, textAlign: 'left', borderBottom: '2px solid #e2e8f0' }}>Fixture</th>
                                <th style={{ padding: 12, textAlign: 'right', borderBottom: '2px solid #e2e8f0' }}>DOM Nodes</th>
                                <th style={{ padding: 12, textAlign: 'right', borderBottom: '2px solid #e2e8f0' }}>Avg Time</th>
                                <th style={{ padding: 12, textAlign: 'right', borderBottom: '2px solid #e2e8f0' }}>Min / Max</th>
                                <th style={{ padding: 12, textAlign: 'right', borderBottom: '2px solid #e2e8f0' }}>Std Dev</th>
                                <th style={{ padding: 12, textAlign: 'center', borderBottom: '2px solid #e2e8f0' }}>Grade</th>
                            </tr>
                        </thead>
                        <tbody>
                            {results.map((result, i) => {
                                ...
                                return (
                                    <tr key={result.name} style={{ backgroundColor: i % 2 === 0 ? 'white' : '#f8fafc' }}>
                                        ...
                                        <td style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0', fontWeight: 600 }}>
                                            {formatTime(result.avgRenderTime)}
                                        </td>
                                        <td style={{ padding: 12, textAlign: 'right', borderBottom: '1px solid #e2e8f0', color: '#666' }}>
                            ...
                            <ScatterChart>
                                ...
                                <Tooltip
                                    formatter={(value, name) => [
                                        typeof value === 'number'
                                            ? (name === 'time' ? `${value.toFixed(2)}ms` : value)
                                            : '-',
                ...
                <div style={{ padding: 16, backgroundColor: 'white', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart>
                            ...
                            />
                            <YAxis fontSize={12} label={{ value: 'ms', angle: -90, position: 'insideLeft' }} />
                            <Tooltip formatter={(value) => value != null ? [`${Number(value).toFixed(2)}ms`] : ['-']} />
                            <Legend />
                            {results.map((result, idx) => {
                                const colors = ['#2563eb', '#22c55e', '#f97316', '#8b5cf6'];
                                const data = result.times.map((time: number, i: number) => ({ iteration: i + 1, [result.name]: time }));
                                return (
                                    <Line
                                        key={result.name}
                                        data={data}
            ...
            {/* Footer */}
            <footer style={{ marginTop: 32, paddingTop: 16, borderTop: '1px solid #e2e8f0', color: '#666', fontSize: 12 }}>
                <p>
                    <strong>Note:</strong> Results may vary based on system load, browser, and hardware.
                    Run benchmarks multiple times for accurate measurements.
                </p>
                <p>
